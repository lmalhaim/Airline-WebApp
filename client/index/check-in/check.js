...
async function verifyBooking(){
    ...
    try{
        const response = await fetch(`http://localhost:${port}/ticket/${pass_id}&${book_ref}`); 
...
async function getPassengersInfo() {
    ...
    try {
        const response = await fetch(`http://localhost:${port}/ticket/${bookRef}`); 
        const jsonData = await response.json();
...
async function verifyBookingStatus() {
    try{    
        const response = await fetch(`http://localhost:${port}/booking/${bookRef}`);
        const jsonData = await response.json();
...
async function getAvailableSeats(){
    try{
        ...
        const response = await fetch(`http://localhost:${port}/seats/${flight_id}/Economy`);
...
async function check_in() {
    ...
    if (verifyStatus){
        ...
        if(!insertStatus){
            ...
            document.location.href = "boarding_submit.html"
...
async function insertBoarding(pass, index) {
    try{
        ...
        const response = await fetch(`http://localhost:${port}/boarding/`, {
            method: "POST",
