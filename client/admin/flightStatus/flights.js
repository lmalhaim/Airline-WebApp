...
async function selectFlights() {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:${port}/list_flights`)
...
async function getPassSeat(pass){
    try{ 
        const response = await fetch(`http://localhost:${port}/seats/${pass.ticket_no}`);  
        const jsonData = await response.json();  
...
async function selectPassengers(flight_id) {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:${port}/passengers/${flight_id}`);
        const jsonData = await response.json();
