...
async function getAircrafts(){
    try{
        alert('porrt' + port); 
        const response = await fetch(`http://localhost:${port}/list_aircrafts`)
...
async function getAirports(){
    try{
        const response = await fetch(`http://localhost:${port}/list_airports`)
        const jsonData = await response.json(); 
...
function insertFlightHandler(){
    ...
    if(isEqual(flight_id.length, 0) || isEqual(scheduled_dep, "Invalid Date") || isEqual(scheduled_arr, "Invalid Date")|| isEqual(seats_avail.length, 0)|| isEqual(dep_gate.length, 0)|| isEqual(bag_claim.length, 0)){
        verification = false; 
        ...
        if(!verifyScheduled(scheduled_arr, scheduled_dep)){
            verification = false; 
            alert("Error: scheduled arrival cannot be before scheduled departure (time and date)"); 
        }
    ...
    if(verification){
        const tempflight = {
            ...
            "arrival_airport":arr_airport,  "aircraft_code":aircraft_code, "available_seats":seats_avail, 
...
async function insertFlight() {
    ...
    try {
        ...
        const response = await fetch("http://localhost:1385/admin/flight", {
            method: "POST",
